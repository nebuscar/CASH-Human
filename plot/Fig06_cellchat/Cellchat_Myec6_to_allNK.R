...
dir_for_cellchat <- "./data/cellchat/cellchat_Myeloidc6_to_allNK.rds"
dir_for_cellchat_CASH <- "./data/cellchat/cellchat_Myeloidc6_to_allNK_CASH.rds"
...
meta_NK <- subset(meta, major_celltype %in% "NK")
meta_Myeloid <- subset(meta, sub_celltype %in% "c07_ Macrophage_ MARCO")
meta_merge <- rbind(meta_NK, meta_Myeloid)

cell.use <-  rownames(meta_merge)
...
saveRDS(cellchat, "./data/cellchat/cellchat_Myeloidc6_to_allNK.rds")
# saveRDS(cellchat, "./data/cellchat/cellchat_Myeloidc6_to_allNK_CASH.rds")
...
targets_use <- all_cells[!all_cells %in% c("c07_ Macrophage_ MARCO")]

p <- netVisual_bubble(cellchat, 
                      sources.use = c("c07_ Macrophage_ MARCO"),
                      targets.use = targets_use, 
                      remove.isolate = F, 
                      sort.by.target = T) + 
  ggtitle("Cellchat_Myeloidc6_to_allNK")
pdf("./fig/Fig06.cellchat/Cellchat_Myeloidc6_to_allNK.pdf", width = 10, height = 10)
print(p)
dev.off()
